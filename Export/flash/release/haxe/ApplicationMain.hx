...
@:dox(hide) class ApplicationMain {
	...
	public static function main () {
		...
		var config = {
			...
			name: "AkibaTrail",
			orientation: "",
			packageName: "com.gamejam.akibatrail",
			version: "1.0.0",
			windows: [
		...
		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed (projectName, null, 800, 600, config);
		#end
		#else
		create (config);
		#end
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("com.gamejam.Main"))) {
			...
			case TInst (t, params):
				...
				if (hasMain) {
					...
					return Context.parse ("@:privateAccess com.gamejam.Main.main ()", Context.currentPos ());
					...
					Context.fatalError ("Main class \"com.gamejam.Main\" has neither a static main nor a constructor.", Context.currentPos ());
					
				}
				
			default:
				
				Context.fatalError ("Main class \"com.gamejam.Main\" isn't a class.", Context.currentPos ());
...
@:keep @:dox(hide) class DocumentClass extends com.gamejam.Main {}
